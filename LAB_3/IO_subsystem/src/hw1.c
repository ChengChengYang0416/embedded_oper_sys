...
bool ifboughtfn(){
  ...
  printf("login_n = %d\n", login_n);
  for (i = 0; i < 10; i++){
    printf("%c", ID_number[i]);
  }
  printf("\n");

  for (i = 0; i < (login_n-1); i++){
    if (check[i][0] == ID_number[0] && check[i][1] == ID_number[1] && check[i][2] == ID_number[2] && check[i][3] == ID_number[3] && check[i][4] == ID_number[4] && check[i][5] == ID_number[5] && check[i][6] == ID_number[6] && check[i][7] == ID_number[7] && check[i][8] == ID_number[8] && check[i][9] == ID_number[9]){
      j++;
    }
  }
  if (j > 0){
