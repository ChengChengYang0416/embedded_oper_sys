...
int day;
int alcohol = 50;
int alcohol_capacity = 50;
useconds_t alcohol_time = 1000000;
useconds_t mask_time = 1000000;
int buied_num = 0;
...
void server_sort(){
    ...
    // sum all kinds of mask
    for (int i = 0; i < num; ++i){
        for (int j = 0; j < data[i][0].type_num; ++j){
            data[i][j].sum = data[i][j].a + data[i][j].b + data[i][j].c;
        }
    }

    // bubble sort
    for (int i = 0; i < num; i++){
        int toIndex = data[i][0].type_num;
        while (toIndex > 1){
            toIndex--;
            for (int j = 0; j < toIndex; j++){
                if (data[i][j].sum < data[i][j+1].sum){
                    bubble_tmp.line = data[i][j].line;
                    for (int k = 0; k < 50 ; k++){
                        bubble_tmp.city[k] = data[i][j].city[k];
                    }
                    bubble_tmp.sum = data[i][j].sum;

                    data[i][j].line = data[i][j+1].line;
                    for (int k = 0; k < 50; k++){
                        data[i][j].city[k] = data[i][j+1].city[k];
                    }
                    data[i][j].sum = data[i][j+1].sum;

                    data[i][j+1] = bubble_tmp;
                    for (int k = 0; k < 50; k++){
                        data[i][j+1].city[k] = bubble_tmp.city[k];
                    }
                    data[i][j+1].sum = bubble_tmp.sum;
                }
            }
        }
    }
    ...
    for (int i = 0; i < num; ++i){
        printf("%s\n", data[i][0].type);
        for (int j = 0; j < data[i][0].type_num; ++j){
            printf("%d %s %d\n", data[i][j].line, data[i][j].city, data[i][j].sum);
        }
    }
...
int main(int argc, char *argv[]){
    ...
    // check if input invalid and receive 
    if(argc != 6){
        printf("Usage: %s <day_of_week> <port> <alcohol capacity> <alcohol time(us)> <mask time(us)>\n", argv[0]);
        return 0;
    }
    day = atoi(argv[1]);
    alcohol = atoi(argv[3]);
    alcohol_capacity = atoi(argv[3]);
    alcohol_time = (useconds_t)atoi(argv[4]);
    mask_time = (useconds_t)atoi(argv[5]);
